...
from Shape2DPainterData import get_shape2d_painter_data_loader
...
def train(epoch):
    ...
    model.train()
    for batch_idx, (prev_canvas, inst, target_obj, act, raw_data) in enumerate(train_loader):
        ...
        loss, rewards = loss_fn(output, target_obj, ref_obj=None)
        policy_loss = (-model.saved_log_probs * Variable(rewards)).sum()
        (loss + policy_loss).backward()
        clip_gradient(optimizer, 0.1)
        optimizer.step()
        color_accuracy, shape_accuracy, row_accuracy, col_accuray = compute_accuracy(output, target_obj, ref_obj=None)
...
def model_test():
    ...
    for batch_idx, (prev_canvas, inst, target_obj, act, raw_data) in enumerate(test_loader):
        ...
        optimizer.zero_grad()
        output = model(inst, prev_canvas, ref_obj=None, target_obj=None)
        prediction = [p.data for p in output]
        ...
        for fail in fails:
            print(raw_data[fail]['instruction'])
            print("{} -> {}".format(list(target_obj.data[fail]), list(target_obj_prediction[fail])))
        print('aka')
...
for epoch in range(1, args.epochs + 1):
    train(epoch)
    torch.save(model.state_dict(), 'painter_model_new/model_{}.pth'.format(epoch))
