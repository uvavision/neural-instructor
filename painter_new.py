...
def train(epoch):
    ...
    for batch_idx, (prev_canvas, inst, target_obj, act, raw_data) in enumerate(train_loader):
        ...
        optimizer.step()
        act_accuracy, color_accuracy, shape_accuracy, row_accuracy, col_accuray = compute_accuracy(output, act, target_obj, ref_obj=None)
        if batch_idx % args.log_interval == 0:
            print('Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.6f} ({:.3f} {:.3f} {:.3f} {} {})'.format(
                epoch, batch_idx * args.batch_size, len(train_loader.dataset),
                       100. * batch_idx / len(train_loader), loss.data[0], act_accuracy, color_accuracy, shape_accuracy, row_accuracy, col_accuray))
